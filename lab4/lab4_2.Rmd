...
tabyl(fish, lakeid)
```
#only works if you have the janitor package installed^
...
```
#get the specific measurments from the data with this command 
...
```
# tolerance is like the error above and below the number you want 
...
```
# & adds more commands, it means "pull only those fish that are AL and >350"
...
```{r}
mammals_compare <- mammals %>% filter(genus=="Lophocebus"|genus=="Erythrocebus"|genus=="Macaca")%>% select(genus, species, mass, gestation, newborn) %>% print
```

```{r}
mammals_compare %>% arrange(newborn)
```
# maura, from the genus Macaca, is the specie with the lowest newborn mass of 389.50. 
