...
```
#!is.na, takes away NA's from our data
...
## 3 Torgersen    52
```
#count acts like both a group_by and summarize together. look at the code below. Different approach, but same result. 
...
penguins%>%
  group_by(species)%>%
  summarize(across(c(bill_length_mm, bill_depth_mm, flipper_length_mm, body_mass_g),
                   na.rm=T, mean))
```
