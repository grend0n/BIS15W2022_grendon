...
deserts %>% 
  ...
  geom_line()+
  geom_point(shape=2)+#gives a custom point(shape=2 is just one of the possible shapes)
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "tomato1", bins=40)+
...
homerange%>%
  ...
  geom_histogram(aes(y = ..density..), fill = "gold", alpha = 0.4, color = "black")+
  labs(title = "Distribution of Body Mass")
```
...
```
#remember, mutate makes a new column that has claculated values from the table.
