...
## 17 Tubulidentata      1
```
#remember, count_by is the same as group_by and summarize
...
# we would like to see it from least to most. Aesthetically.
...
life_history %>% 
  ...
  geom_col()+coord_flip()+scale_y_log10()
```

![](lab10_1_files/figure-html/unnamed-chunk-9-1.png)<!-- -->
#the reorder function alligns them to least to most 
...
#if you don't have scale_y_log10, then it will turn out as an unhelpful graph
...
#the labs comand allows us to label our axis and title
...
```r
homerange%>%
  count(thermoregulation, "endotherm", "ectotherm")
```

```
## # A tibble: 2 × 4
##   thermoregulation `"endotherm"` `"ectotherm"`     n
##   <chr>            <chr>         <chr>         <int>
## 1 ectotherm        endotherm     ectotherm       191
## 2 endotherm        endotherm     ectotherm       378
```
...
homerange%>%
 ggplot(aes(x=thermoregulation, fill=thermoregulation))+
  geom_bar()+
  labs(title = " Number of Ectotherms and Endotherms",
       x = "Thermoregulation",
       y = "n") +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
...
homerange %>% 
  ...
  geom_point(size=1, color="darkorchid4", alpha=0.6)+
